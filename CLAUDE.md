...
1. **Design tokens first.** Phase 0 produces `design-tokens` (Tailwind theme extension): a named palette, a type scale, spacing scale. Every component uses tokens; hex values outside the theme file are a lint error (email templates excepted — see DECISIONS.md).

   **v3 palette (owner decision, 2026-07-26 — supersedes the "~5 colors, no gradients" rule):** three brand hues — **blue** (`brand`, interactive), **violet** (`violet`, data visualisation), **gold** (`gold`, reserved for the user's own inputs, annotations, estimates and hand-kept data). Gradients are part of the identity, not decoration: `bg-grad-brand`, `bg-grad-gold`, `bg-grad-surface`, `bg-grad-shell`, `text-grad-brand`, and the chart area fills. Green/red remain reserved for gain/loss and may never be used as brand colours.
...
4. **Readable density** (v3, owner decision 2026-07-26 — was "density over whitespace"): body text 15px, line-height 1.55, nothing below 12px except a single superscript marker; table rows `px-4 py-2.5`. Dense enough to compare a statement, never so tight it strains. Indian/Pakistani number formatting respected: lakh/crore display toggle for INR/PKR values (1,23,45,678 grouping), thousands/millions for USD.
5. **Color encodes meaning**: positive/negative deltas (green/red, colorblind-safe — the sign character always renders too), gold for the user's own inputs/annotations/estimates, blue for interactive, violet for data visualisation. Gradients are permitted and encouraged on brand surfaces (hero cards, buttons, shell, chart fills, the RangeBand); they must never sit behind a number that has to be read. No glassmorphism, no glow.
   **Every clickable element must acknowledge the click**: `.pressable` / `.pressable-row` for the press dip, `SubmitButton` for in-flight server actions, and a route-level `loading.tsx` so navigation is never silent.
