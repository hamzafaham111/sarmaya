...
export function DataTable({
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-line bg-surface">
      <table
        className="w-full min-w-max text-[14px] leading-[1.4]"
      ...
      >
        <thead>
          <tr className="border-b border-line">
            ...
            {columns.map((col) => (
              <th
                key={col.key}
                className={`px-4 py-2.5 font-medium text-ink-muted ${col.numeric === false ? "text-left" : "text-right"}`}
              >
                {col.header}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row) => (
            ...
            >
              <td
                className={`sticky left-0 bg-surface px-4 py-2.5 text-left ${row.emphasis ? "font-medium text-ink" : "text-ink-muted"}`}
              >
                {row.label}
              </td>
              {row.cells.map((cell, i) => (
                <td
                  key={columns[i]?.key ?? i}
                  className={`font-numeric px-4 py-2.5 tabular-nums ${columns[i]?.numeric === false ? "text-left" : "text-right"} ${row.emphasis ? "font-medium text-ink" : "text-ink"}`}
                >
                  {cell ?? DASH}
                </td>
              ))}
